...
Solution::Solution(const std::shared_ptr<Instance>& instance) {
    ...
    sortie_stages.resize(instance->num_node, SortieInfo{-1, -1, -1, -1});
}

std::shared_ptr<Solution> Solution::copy() {
    auto new_solution = std::make_shared<Solution>(instance);
...
double Solution::get_truck_cost(const int& index1, const int& index2) const {
    double cost = 0;
...
void Solution::calculate_objective() {
    double obj = 0;
    // truck tour length
...
void Solution::print_solution_light() {
...
void Solution::check_duplicates(const std::vector<int>& vec) {
    if (!vec.empty()) {
...
bool Solution::feasibility_check() {
...
void Solution::print_truck_tour() {
...
void Solution::sortie_feasibility_check() {
    ...
    for (int i = 0; i <= instance->num_node; i++) {
        ...
        if (!exist(truck_route, i) && !exist(served_by_drone, i) && visited[i]) {
...
void Solution::remove_drone_customer(const int c) {
...
void Solution::remove_customer_loop_no_battery(int customer_to_remove) {
...
void Solution::remove_customer(const int customer_to_remove) {
    if (isElementInArray(served_by_drone, customer_to_remove)) {
        remove_drone_customer(customer_to_remove);
    }
    else {
...
void Solution::remove_truck_index_loop_no_battery(const int index) {
    if (truck_route[index - 1] == truck_route[index + 1]) {
        return;
    }
...
bool Solution::remove_truck_index(const int index) {
    if (truck_route[index - 1] == truck_route[index + 1]) {
        return false;
    }
    const int customer = truck_route[index];
...
void Solution::remove_string(const int start_index, const int remove_size) {
    ...
    if (start_index + remove_size - 1 > truck_route.size() - 2) {
        ...
        if (const int last_index_sec = 1 + remove_size - truck_route.size() + start_index; last_index_sec >= start_index) {
            ...
            if (i_mid_of == -1 && j_mid_of == -1) {
                ...
                if (new_truck_cost_of_i_mid <= instance->e - instance->sr) {
                    ...
                    if (sortie_increased_cost + increased_cost_at_j < -1e-3) {
                        ...
                        calculate_objective();
                        std::cout << "Solution after the swap: " << std::endl;
...
void Solution::easy_sortie_update() {
    for (const int& d : served_by_drone) {
...
void Solution::swap_op(const int i, const int j) {
    const int temp = truck_route[i];
    truck_route[i] = truck_route[j];
    truck_route[j] = temp;
}

void Solution::revisit_loop_no_battery() {
    bool revisitable = true;
tag:
    while (revisitable) {
        revisitable = false;
        for (int index = 1; index < truck_route.size() - 2; index++) {
            for (int rep_index = index + 1; rep_index < truck_route.size() - 1; rep_index++) {
                // do:
                if (truck_route[index] == truck_route[rep_index]) {
                    continue;
                }
                ...
                if (!instance->heavy_bool[truck_route[rep_index]]) {
                    ...
                    if (!is_l && !is_r) {
                        ...
                        for (int launch_index = rep_index - 1; launch_index >= earliest_launch_index;
                            ...
                            const double left_drone_trip = instance->tau_prime[truck_route[launch_index]][
                        ...
                        for (int rendezvous_index = rep_index + 1; rendezvous_index <= earliest_rendezvous_index
                            ...
                            const double right_drone_trip = instance->tau_prime[truck_route[index]][
                                    truck_route[
                                        rep_index]]
                        ...
                        const double l_of_new_drone_trip = instance->tau_prime[truck_route[index]][l_of]
                            + instance->tau_prime[l_of][truck_route[sortie_stages[l_of].end_index]];


                        const double l_of_new_truck_trip = sortie_stages[l_of].truck_trip
                            - instance->tau[truck_route[rep_index]][truck_route[rep_index + 1]]
                            + instance->tau[truck_route[index]][truck_route[rep_index + 1]];
                        ...
                        for (int launch_index = rep_index - 1; launch_index >= earliest_launch_index_free_set;
                            ...
                            const double rep_drone_trip = instance->tau_prime[truck_route[
                                    launch_index]]
                                [truck_route[rep_index]]
                                + instance->tau_prime[truck_route[rep_index]][truck_route[index]];
                        ...
                        const double rep_new_truck_trip = sortie_stages[l_of].truck_trip
                            - instance->tau[truck_route[rep_index]][truck_route[rep_index + 1]]
                            + instance->tau[truck_route[index]][truck_route[rep_index + 1]];

                        const double rep_new_drone_trip = instance->tau_prime[truck_route[index]][
                                truck_route[rep_index]]
                        ...
                        for (int launch_index = rep_index - 1; launch_index >=
                            ...
                            const double l_of_drone_trip = instance->tau_prime[truck_route[
                                    launch_index]][l_of]
                                + instance->tau_prime[l_of][truck_route[index]];
                    ...
                    if (!is_l && is_r) {
                        ...
                        // drone trip of r_of
                        const double new_r_of_drone_trip = instance->tau_prime[truck_route[sortie_stages[r_of].
                                start_index]][r_of]
                            + instance->tau_prime[r_of][truck_route[index]];
                        // okay.
                        ...
                        for (int rendezvous_index = rep_index + 1; rendezvous_index <= earliest_rendezvous_index
                            ...
                            const double rep_drone_trip = instance->tau_prime[truck_route[index]][
                                    truck_route[rep_index]]
                        ...
                        double best_second_case_saved_cost = -1e9;
                        const double rep_new_truck_trip = sortie_stages[r_of].truck_trip
                            - instance->tau[truck_route[rep_index - 1]][truck_route[rep_index]]
                            + instance->tau[truck_route[rep_index - 1]][truck_route[index]];

                        const double rep_new_drone_trip = instance->tau_prime[truck_route[sortie_stages[r_of].
                        ...
                        for (int rendezvous_index = rep_index + 1; rendezvous_index <= earliest_rendezvous_index
                            ...
                            const double r_of_drone_trip = instance->tau_prime[truck_route[index]][
                                    r_of]
    ...
    while (revisitable) {
        ...
        for (int index = 1; index < truck_route.size() - 3; index++) {
            ...
            for (int rep_index = index + 2; rep_index < truck_route.size() - 1; rep_index++) {
                ...
                // do:
                if (exist(duplicateIndices, rep_index)) {
                    continue;
                }
                if (truck_route[rep_index - 1] == truck_route[index] || truck_route[rep_index + 1] == truck_route[
...
void Solution::remove_sortie(const int c) {
...
int Solution::remove_highest_rate_sortie(const int c) {
    const int start_index = sortie_stages[c].start_index;
    const int end_index = sortie_stages[c].end_index;
    const int start_node = truck_route[start_index];
    const int end_node = truck_route[end_index];
...
void Solution::duplicate_node_cleaner() {
    std::unordered_map<int, int> countMap;
    std::vector<int> duplicateIndices;
...
void Solution::invalid_occurrence() {
    ...
    for (int i = 1; i < instance->num_node; ++i) {
        if (occurrences[i].size() > 1) {
            for (int index = 1; index < occurrences[i].size(); index++) {
                ...
                for (const int d : served_by_drone) {
                    if (sortie_stages[d].start_index == occurrences[i][index] || occurrences[i][index] == sortie_stages[
                        d].end_index) {
                        // in middle.
                        invalid = false;
                        break;
...
void Solution::sortie_invalid_occurrence() {
